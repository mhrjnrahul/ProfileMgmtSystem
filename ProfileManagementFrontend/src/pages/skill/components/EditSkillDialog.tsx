...
export default function EditSkillDialog({ skill }: { skill: SkillResponse }) {
  ...
  defaultValues: {
    name: skill.name,
    proficiencyLevel: skill.proficiencyLevel, // this registers the field
  },
});

  const mutation = useMutation({
    mutationFn: (data: FormData) => {
  console.log("Sending to API:", { ...data, id: skill.id });
  return updateSkill(skill.id, { ...data, id: skill.id });
},
    onSuccess: () => {
        ...
        <form onSubmit={handleSubmit((data) => mutation.mutate(data))} style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 4 }}>
          ...
          {mutation.isError && (
            <div style={{ background: "#FEF2EE", border: "1px solid #F5C4B5", borderRadius: 8, padding: "8px 12px" }}>
              <p style={{ fontSize: 12, color: "#C4735A" }}>Failed to update skill.</p>
            </div>
          )}
